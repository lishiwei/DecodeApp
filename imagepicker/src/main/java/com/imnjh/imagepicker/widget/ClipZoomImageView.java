...
public class ClipZoomImageView extends androidx.appcompat.widget.AppCompatImageView implements
    ...
    public ClipZoomImageView(Context context, int aspectX, int aspectY) {
        this(context);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isCanDrag) {
                    if (getDrawable() != null) {
                        ...
                        if (rectF.width() <= getRectWidth()) {
                            dx = 0;
                        }
                        if (rectF.height() <= getRectHeight()) {
                            dy = 0;
                        }
    ...
    public void onGlobalLayout() {
        if (once) {
            ...
            int frameSizeX = getRectWidth();
            int frameSizeY = getRectHeight();

            if (drawableW > frameSizeX && drawableH < frameSizeY) {
                scale = 1.0f * frameSizeY / drawableH;
            } else if (drawableH > frameSizeY && drawableW < frameSizeX) {
                scale = 1.0f * frameSizeX / drawableW;
            } else if (drawableW > frameSizeX && drawableH > frameSizeY) {
                float scaleW = frameSizeX * 1.0f / drawableW;
                float scaleH = frameSizeY * 1.0f / drawableH;
            ...
            if (drawableW < frameSizeX && drawableH > frameSizeY) {
                ...
                float scaleW = 1.0f * frameSizeX / drawableW;
                float scaleH = 1.0f * frameSizeY / drawableH;
                scale = Math.max(scaleW, scaleH);
            }
    ...
    public Bitmap clip() {
        ...
        Bitmap image = Bitmap.createBitmap(bitmap, getHorizontalPadding(), getHVerticalPadding(), width, height);
        return image;
    }
    ...
    private void checkBorder() {
        ...
        if (rect.width() + 0.01 >= getRectWidth()) {
            if (rect.left > getHorizontalPadding()) {
                deltaX = -rect.left + getHorizontalPadding();
            }

            if (rect.right < width - getHorizontalPadding()) {
                deltaX = width - getHorizontalPadding() - rect.right;
            }
        }

        if (rect.height() >= getRectHeight()) {
    ...
    public int getRectWidth() {
        return getWidth() - 2 * getHorizontalPadding();
    }

    public int getRectHeight() {
        return getHeight() - 2 * getHVerticalPadding();
    }
    ...
    public int getHorizontalPadding() {
        if (aspectX >= aspectY) {
            ...
            Log.d("ClipZoomImageView", "getHorizontalPadding: "+height);
            Log.d("ClipZoomImageView", "getHorizontalPadding: "+width);
            Log.d("ClipZoomImageView", "getHorizontalPadding: "+getWidth());
            if (getWidth()-width<=0)
            {
                return 0;
            }
